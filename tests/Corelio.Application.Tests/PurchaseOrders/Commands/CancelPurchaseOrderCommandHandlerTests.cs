...
public class CancelPurchaseOrderCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WithDraftOrder_TransitionsToCancelled()
    {
        // Arrange
        ...
        // Act
        var result = await _handler.Handle(new CancelPurchaseOrderCommand(po.Id), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WithSubmittedOrder_TransitionsToCancelled()
    {
        // Arrange
    ...
    [Fact]
    public async Task Handle_WithApprovedOrder_ReturnsConflictError()
    {
        // Arrange
        var po = CreateOrderWithStatus(PurchaseOrderStatus.Approved);
        _repoMock.Setup(x => x.GetByIdAsync(po.Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(po);
        ...
        po.Status.Should().Be(PurchaseOrderStatus.Approved); // unchanged
    }
    ...
    public async Task Handle_WhenOrderNotFound_ReturnsNotFoundError()
    {
        ...
        // Act
        var result = await _handler.Handle(new CancelPurchaseOrderCommand(id), CancellationToken.None);
